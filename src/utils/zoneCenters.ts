...
export const ZONE_CENTER_OVERRIDES: Record<string, LatLng> = {
...
export function zoneCenter(zone: Zone | null | undefined): LatLng | null {
  if (!zone) return null
  return ZONE_CENTER_OVERRIDES[zone.name] ?? { lat: Number(zone.lat), lng: Number(zone.lng) }
}
